...
interface Order {
  ...
  erhalteneBestellungen: number;
  verteilteAnzahl: number;
  aktuellerBestand: number;
  mindestBestand: number;
  maxBestand: number;
  prioritaet: string;
  verteilungseinheit?: string;
  bestandseinheit?: string;
}

const ReportsPage = ({ orders, onBack }: { orders: Order[]; onBack: () => void }) => {
  ...
  const kategorieStats: { [key: string]: { bestellt: number; verteilt: number; bestand: number } } = {};
...
const UltraModernHospitalApp = () => {
  ...
  const getLagerStatus = (order: Order) => {
